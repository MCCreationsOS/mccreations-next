...
export default function Comments({mapSlug, comments}) {
    ...
    return (
        <div className='centered_content'>
                ...
                <h2>Comments</h2>
                {(commentsState && commentsState.length > 0 && commentsState[0].comment) ? commentsState.map((comment, idx) => <CommentCard key={idx} comment={comment} />) : <><div className="no_comments"><h3>None Yet!</h3><p>Be the first to comment!</p></div></>}
        </div>
    )
}