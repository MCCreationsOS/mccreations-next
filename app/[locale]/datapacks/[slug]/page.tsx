...
export async function generateMetadata(props: { params: Promise<{locale: string, slug: string}> }, parent: ResolvingMetadata): Promise<Metadata> {
    ...
    return {
        title: t('Pages.Datapacks.slug.Metadata.title', {title: map.title, content_type: t('datapack', {count: 1}), creator: (map.creators && map.creators[0]) ? map.creators[0].username : "", minecraft_version: (map.files && map.files.length > 0) ? ( typeof map.files[0].minecraftVersion === 'object' ? map.files[0].minecraftVersion.filter((version: string) => version !== "").join(", ") : map.files[0].minecraftVersion ?? "") : ""}),
